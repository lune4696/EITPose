...
class ExperimentRunner:
    def __init__(
        ...
        experiment_author="anonymous",
        path_to_data=path_to_root / "src" / "data" / "processed_data",
    ):
            ...
            for evaluation_type, rounds in experiment["evaluations"].items():
                ...
                if experiment["experiment_params"]["evaluator_type"] == "classifier":
                    (
                        ...
                        conf_matrix_list,
                        save_location,
                    ) = etr_exp.run_experiment()
                    combined_conf_matrix = np.sum(np.stack(conf_matrix_list), axis=0)
                    normalized_conf_matrix = (
                        np.transpose(
                            np.transpose(combined_conf_matrix)
                            / np.sum(combined_conf_matrix, axis=1)
                        )
                        * 100.0
                    )
                    ...
                    drop_labels = experiment["experiment_params"]["y_cols"][
                        "gesture_label"
                    ]["exclude_values_no_drop"] + [12]
                    labels = np.delete(labels, drop_labels)
                    disp = ConfusionMatrixDisplay(
                        confusion_matrix=normalized_conf_matrix, display_labels=labels
                    )
                    fig, ax = plt.subplots(
                        figsize=(10, 10)
                    ...
                    plt.xticks(
                        rotation=45
                    )  # Rotate labels by 45 degrees, or any angle you prefer

                    plt.tight_layout()
                    plt.savefig(
                        str(
                            output_folder
                            / (str(i) + "_" + str(evaluation_type) + ".png")
                        )
                    )
                    plt.show()
                    output = "\n".join(
                    ...
                    (
                        ...
                        error_list_by_joint,
                        std_list_by_joint,
                        error_list_by_participant,
                        std_list_by_participant,
                        save_location,
                    ) = etr_exp.run_experiment()
                    output = "\n".join(
                        [
                            ...
                            f"Error List by Participant {
                                str(error_list_by_participant)
                            }",
                            f"Std List by by Participant {
                                str(std_list_by_participant)
                            }",
                            f"Model Locations: {str(save_location)}",
                        ]
                    )
