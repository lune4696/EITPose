...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.gl_widget.update_overlay_text(
            "title", "Mediapipe Handpose (Actual)", 10, 30, 14
        )
        self.gl_widget.sizeHint = lambda: pg.QtCore.QSize(
            800, 400
        ...
        pen_colors = ["r", "g", "b", "y", "m",
                      "c", (255, 165, 0), (128, 0, 128)]
        self.eit_plots = [self.eit_plot.plot(
            pen=pen_colors[i]) for i in range(8)]
        ...
        self.gesture_plots = [
            self.gesture_plot.plot(pen=pen_colors[i]) for i in range(2)
        ]

        self.timeline = QSlider(Qt.Horizontal)
        self.timeline.setTickPosition(QSlider.NoTicks)  # No tick marks
        # Set the single step to 1 for fine control
    ...
    def load_handpose_data(self, file_name):
        ...
        if "mphands_data" in data.columns:
            self.handpose_actual = (
                np.stack(data["mphands_data"].tolist()).reshape(-1, 21, 3) * 10
            )
        else:
        ...
        self.eit_data = np.stack(data["eit_data"].tolist()) - np.mean(
            np.stack(data["eit_data"].tolist()), axis=0
        )
        ...
        if self.gesture_actual is not None and self.handpose_actual is not None:
            self.update_plot(
                self.gl_widget,
                self.handpose_actual[0, :, 0],
                self.handpose_actual[0, :, 1],
                self.handpose_actual[0, :, 2],
                0,
            )
            self.gesture_plots[0].setData(
                self.timestamps[0:100], self.gesture_actual[0:100]
            )
        ...
        for i in range(8):
            self.eit_plots[i].setData(
                self.timestamps[0:100], self.eit_data[0:100, 40 + i]
            )
    ...
    def timeline_value_changed(self, value):
        ...
        if self.gesture_actual is not None and self.handpose_actual is not None:
            self.update_plot(
                self.gl_widget,
                self.handpose_actual[value, :, 0],
                self.handpose_actual[value, :, 1],
                self.handpose_actual[value, :, 2],
                self.gesture_actual[value],
            )
            self.gesture_plots[0].setData(
                self.timestamps[value: min(
                    100 + value, len(self.timestamps) - 1)],
                self.gesture_actual[value: min(
                    100 + value, len(self.timestamps) - 1)],
            )
        elif self.gesture_actual is not None:
            self.update_plot(
                self.gl_widget, None, None, None, self.gesture_actual[value]
            )
            self.gesture_plots[0].setData(
                self.timestamps[value: min(
                    100 + value, len(self.timestamps) - 1)],
        ...
        for i in range(8):
            self.eit_plots[i].setData(
                self.timestamps[value: min(
                    100 + value, len(self.timestamps) - 1)],
                self.eit_data[
                    value: min(100 + value, len(self.timestamps) - 1), 40 + i
                ],
            )
    ...
    def update_plot(self, plot, x, y, z, gesture):
        ...
        if x is not None:
            ...
            scatter = gl.GLScatterPlotItem(
                pos=np.column_stack((x, y, z)), color=(0.5, 0.5, 0.5, 1.0), size=15
            )  # Increase size to 0.1
            ...
            for i in range(len(connections)):
                ...
                line = gl.GLLinePlotItem(
                    pos=np.column_stack((line_x, line_y, line_z)),
                    color=color,
                    width=10.0,
                )
                plot.addItem(line)
        ...
        if gesture is not None:
            plot.update_overlay_text(
                "gesture",
                "Gesture: "
                + str(gesture)
                + " "
                + str(
                    self.gesture_dictionary[gesture]
                    if gesture in self.gesture_dictionary
                    else None
                ),
                plot.width() - 300,
                plot.height() - 150,
            ...
            plot.update_overlay_text(
                "gesture", "Gesture: None", plot.width() - 300, plot.height() - 150, 14
            )
        ...
        if self.playing:
            ...
            if self.annotation_enabled:
                if self.index_value < len(self.labeled_data):
                    self.labeled_data[self.index_value] = (
                        None
                        if self.gesture_group.checkedId() == 18
                        else self.gesture_group.checkedId()
                    )
                else:
                    self.play_button.setChecked(False)
                    self.playing = False
            ...
            if self.annotation_enabled and self.gesture_group.checkedId() == 18:
                self.start_timestamp = self.timestamps[self.index_value]
            if (
                self.annotation_enabled
                and self.gesture_group.checkedId() != 18
                and self.timestamps[self.index_value] - self.start_timestamp > 1.5
            ):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_F:
            ...
            super().keyPressEvent(
                event
            )  # Call the parent class method to ensure default behavior
